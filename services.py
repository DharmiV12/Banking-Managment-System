...
def create_account(db, name:str, phone_no:BigInteger, address:str, age:int):
    try:
        acc_details = Account(name=name, phone_no=phone_no, address=address, age=age, balance=0.0)
...
def deposit(db, id:int, amount:int):
    try:
        ...
        if not user_model.id:
            ...
            raise HTTPException(status_code=404, detail="Account ID is not exist for deposit")
        
        user_model.balance += amount

        stmt = Transaction(acc_id = user_id, balance_after = user_model.balance, amount=amount, description = "DEPOSIT")
...
def withdraw(db, id:int, amount:int):
    try:
        ...
        if not user_id:
            ...
            raise HTTPException(status_code=404, detail="Account ID is not exist for withdrawal")
        ...
        stmt = Transaction(acc_id = user_id, balance_after = user_model.balance, amount=amount, description = "WITHDRAWAL")
...
def get_account(db, id:int):
    try:
        ...
        if not user_id:
           logging.warning("This ID is not exist to fetched account details, enter valid ID")
           raise HTTPException(status_code=404, detail="Account ID is not exist")
...
def transaction_history(db, id:int):
    try:
        ...
        if not user_id:
           logging.warning("This ID is not exist, enter valid ID to show transactoin history")
           raise HTTPException(status_code=404, detail="Account ID is not exist")

        stmt = select(Transaction).where(Transaction.acc_id == id).order_by(Transaction.updated_at.desc())
